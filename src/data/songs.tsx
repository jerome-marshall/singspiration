import { Ending, Intro, Line, Song, Stanza } from "../types"

export const vaanangalayum: Song = {
  title: "வானங்களையும்",
  stanzas: [
    {
      lines: [
        { text: "வானங்களையும் அதின் சேனைகளையும்" },
        { text: "உண்டாக்கிய நீர் ஒருவரே கர்த்தர்", times: 2 },
        { text: "பூமியையும் அதில் உள்ளவைகளும்" },
        { text: "உண்டாக்கிய நீர் ஒருவரே கர்த்தர்" },
        { text: "சமுத்திரமும் அதில் உள்ளவைகளும்" },
        { text: "காப்பாற்றும் நீர் நீர் ஒருவரே கர்த்தர்" },
      ],
    },
    {
      lines: [
        { text: "தண்ணீர்களையும் தம் கையால் அளந்து" },
        { text: "பூமியின் மண்ணை மரக்காலால் அடக்கி", times: 2 },
        { text: "மலைகளை பிடித்து தம் கையில் எடுத்து" },
        { text: "பர்வதங்களை தராசில் நிறுத்தும்", times: 2 },
      ],
    },
    {
      lines: [{ text: "நீர் ஒருவரே...", times: 4 }],
      noNextChorus: true,
    },
    {
      lines: [
        { text: "சாவாமை உள்ளவர் அவர் ஒருவரே" },
        { text: "சர்வத்தை ஆள்பவர் அவர் ஒருவரே", times: 2 },
        { text: "வானம் படைத்தவர் இந்த பூமி படைத்தவர்" },
        { text: "நட்சத்திரங்களை" },
        { text: "பெயர் சொல்லி அழைத்தவர்", times: 2 },
      ],
    },
  ],
  chorus: {
    lines: [
      { text: "நீர் ஒருவரே கர்த்தர்" },
      { text: "நீர் ஒருவரே கர்த்தர்" },
      { text: "நீர் ஒருவரே கர்த்தர்" },
      { text: "நீர் ஒருவரே", times: 2 },
    ],
  },
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
}

export const sugamVanthadu: Song = {
  title: "சுகம் வந்தது",
  stanzas: [
    {
      lines: [
        { text: "சுகம் வந்தது வந்தது மண்ணுலகில்" },
        { text: "சுடர் தந்தது தந்தது பொன்னுலகில்", times: 2 },
        { text: "நிரந்தர இன்பம் தந்தவரே" },
        { text: "இறைவனின் வடிவம் இயேசு நாதரே", times: 2 },
      ],
    },
    {
      lines: [
        { text: "அல்லேலூயா நாமம் பாடி" },
        { text: "காற்று எங்கும் வீசட்டும்" },
        { text: "அன்பு பண்பு பாசம் பொங்கி" },
        { text: "தேவன் நம்மை ஆளட்டும்", times: 2 },
      ],
    },
    {
      lines: [
        { text: "எல்லோரும் அவர் புகழ் பாடுங்கள்" },
        { text: "எந்நாளும் அவரையே தேடுங்கள்", times: 2 },
        { text: "எங்கும் எங்கும் உம் இராஜங்கம்" },
        { text: "எங்கும் இன்பம் இன்பம் நீர் தாராயோ" },
      ],
    },
    {
      lines: [
        { text: "இனிமை சுரக்கும் இதயம் மணக்கும்" },
        { text: "மேசியா இராஜா கீதம் நிதம் பாடுவோம்" },
        { text: "கதவு திறக்கும் கருணை பிறக்கும்" },
        { text: "இயேசையா ஞான கீதம் நிதம் காணுவோம்" },
      ],
    },
    {
      lines: [
        { text: "தேவ மைந்தன் இயேசுவை" },
        { text: "தேசம் எங்கும் போற்றவே" },
        { text: "பாரம் எல்லாம் சுமப்பவர்" },
        { text: "பாவம் எல்லாம் தீரவே" },
      ],
    },
    {
      lines: [
        { text: "புது வெள்ளி வான்பிறை தொட்டில்" },
        { text: "அது அன்பின் தாமரை மொட்டில்", times: 2 },
        { text: "வரும் பாலகனை மரி பாலகனை" },
        { text: "என்றும் பணிந்திடுவோம்" },
      ],
    },
    {
      lines: [{ text: "அல்லேலூயா அல்லேலூயா" }],
    },
  ],
  chorus: null,
  chorusFrequency: "none",
  startWithChorus: false,
}

export const yesuvinAnbinai: Song = {
  title: "இயேசுவின் அன்பினை",
  stanzas: [
    {
      lines: [
        { text: "இயேசுவின் அன்பினை அறிவித்திட" },
        { text: "இணைந்தே செயல்படுவோம்" },
        { text: "சுவிசேஷ நற்செய்தி கூறிட" },
        { text: "விரைந்தே புறப்படுவோம்", times: 2 },
      ],
    },
    {
      lines: [
        { text: "நினிவேயின் ஜனங்களுக்காக" },
        { text: "நம் தேவன் பரிதவித்தார்", times: 2 },
        { text: "தீர்க்கன் யோனாவையோ" },
        { text: "அவர் அனுப்பி எச்சரித்தார்", times: 2 },
        { text: "இலட்சத்திற்காக பரிதபித்தார்" },
        { text: "கோடிகட்காக கலங்கிடாரோ", times: 2 },
      ],
    },
    {
      lines: [
        { text: "காலங்கள் கடந்திடுதே" },
        { text: "நம் வேதமும் நிறைவேறுதே", times: 2 },
        { text: "இயேசுவின் வருகை இதோ" },
        { text: "அதி சமீபமாகிறதே", times: 2 },
        { text: "இளைஞர் கூட்டம் இயேசுவுக்காய்" },
        { text: "நற்செய்தி சுமந்து புறப்படுவோம்", times: 2 },
      ],
    },
  ],
  chorus: {
    lines: [
      { text: "நம் பாரதம் நம் தாயகம்" },
      { text: "கர்த்தரை அறியட்டுமே" },
      { text: "நம் தாய் மண்ணும் நம் தலைமுறையும்" },
      { text: "இயேசுவை அறியட்டுமே", times: 2 },
    ],
  },
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
}

export const varugaiKaana: Song = {
  title: "வருகை காண விழிகள்",
  chorus: {
    lines: [
      { text: "எப்போது வருவீர் என்று - நான்" },
      { text: "ஏங்கி தவித்து இருந்தேன்" },
      { text: "சீக்கிரம் வருவீர் என்று அறிவேன்", times: 2 },
    ],
  },
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
  stanzas: [
    {
      lines: [
        { text: "வருகை காண விழிகள்" },
        { text: "இரண்டும் ஏங்குதே" },
        { text: "இரட்சகர் முகத்தை பார்க்க" },
        { text: "மனமும் ஏங்குதே", times: 2 },
      ],
    },
    {
      lines: [
        { text: "வருகை காண விழிகள்" },
        { text: "இரண்டும் ஏங்குதே" },
        { text: "இரட்சகர் முகத்தை பார்க்க" },
        { text: "மனமும் ஏங்குதே" },
      ],
      noNextChorus: true,
    },
    {
      lines: [
        { text: "யுத்தங்கள் செய்தியை கேட்கிறேன்" },
        { text: "பஞ்சங்கள் செய்தியை கேட்கிறேன்" },
        { text: "பூமி அதிர்வுகள் உணர்கிறேன்" },
        { text: "வாதை நோய்களை காண்கிறேன்", times: 2 },
        { text: "அன்பு தனிவதை காண்கிறேன்" },
        { text: "விசுவாசம் குறைவதை காண்கிறேன்..." },
      ],
    },
    {
      lines: [
        { text: "வருகை காண விழிகள்" },
        { text: "இரண்டும் ஏங்குதே" },
        { text: "இரட்சகர் முகத்தை பார்க்க" },
        { text: "மனமும் ஏங்குதே" },
      ],
      noNextChorus: true,
    },
    {
      lines: [
        { text: "தற்பிரியரையும் காண்கிறேன்" },
        { text: "பணபிரியரையும் காண்கிறேன்" },
        { text: "அறிவின் பெருக்கத்தை பார்க்கிறேன்" },
        { text: "அழிவின் நெருக்கத்தை பார்க்கிறேன்", times: 2 },
        { text: "கட்டளை மீறல் பார்க்கிறேன்" },
        { text: "வேதம் நிறைவேறல் பார்க்கிறேன்..." },
      ],
    },
    {
      lines: [
        { text: "வருகை காண விழிகள்" },
        { text: "இரண்டும் ஏங்குதே" },
        { text: "இரட்சகர் முகத்தை பார்க்க" },
        { text: "மனமும் ஏங்குதே" },
      ],
    },
    {
      lines: [
        { text: "வருகை காண விழிகள்" },
        { text: "இரண்டும் ஏங்குதே" },
        { text: "இரட்சகர் முகத்தை பார்க்க" },
        { text: "மனமும் ஏங்குதே" },
      ],
      noNextChorus: true,
    },
  ],
}

export const oruSanthathiAvarai: Song = {
  title: "ஒரு சந்ததி அவரை சேவிக்கும்",
  chorus: {
    lines: [
      { text: "சந்ததி... சந்ததி... சந்ததி..." },
      { text: "ஒரு சந்ததி அவரை சேவிக்கும்..." },
    ],
  },
  stanzas: [
    {
      lines: [
        { text: "ஒரு சந்ததி அவரை சேவிக்கும்" },
        { text: "தலைமுறை தலைமுறையாக" },
        { text: "அது இயேசுவின்‌ சந்ததி எனப்படும்‌", times: 2 },
        { text: "சந்ததி... சந்ததி... சந்ததி", times: 2 },
      ],
      noNextChorus: true,
    },
    {
      lines: [
        { text: "தேவனைத்‌ தேடும்‌ சந்ததி" },
        { text: "தேவ சமூகத்தை நாடும்‌ சந்ததி", times: 2 },
        { text: "உலகத்தைக்‌ கலக்கும்‌ சந்ததி - அது" },
        { text: "உன்னத தேவனின்‌ சந்ததி", times: 2 },
      ],
    },
    {
      lines: [
        { text: "வாரியிரைக்கும்‌ சந்ததி - வாழ்வில்‌" },
        { text: "வறுமையைக்‌ காணா சந்ததி", times: 2 },
        { text: "வசனத்தை விதைக்கும்‌ சந்ததி - அது" },
        { text: "வானகம்‌ செல்லும்‌ சந்ததி", times: 2 },
      ],
    },
    {
      lines: [
        { text: "இருளில்‌ இருந்த சந்ததி - ஞான" },
        { text: "ஒளியாய்‌ மாறின சந்ததி", times: 2 },
        { text: "இயேசுவை அறிவிக்கும்‌ சந்ததி - இது" },
        { text: "இரக்கம்‌ நிறைந்த சந்ததி", times: 2 },
      ],
      noNextChorus: true,
    },
    {
      lines: [
        { text: "சந்ததி... சந்ததி... சந்ததி..." },
        { text: "ஒரு சந்ததி அவரை சேவிக்கும்..." },
        { text: "சந்ததி... சந்ததி... சந்ததி..." },
        { text: "ஒரு சந்ததி அவரை சேவிக்கும்..." },
      ],
      noNextChorus: true,
    },
  ],
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
}

export const oivunaalKarpanai: Song = {
  title: "ஓய்வுநாள் கற்பனை",
  stanzas: [
    {
      lines: [
        { text: "ஓய்வுநாள் கற்பனை என்ன சொல்லுது" },
        { text: "உழைப்பில்லாத ஓய்வுபாவம் என்று சொல்லுது", times: 2 },
        { text: "சும்மா இருப்பதே சுகம் என்று எண்ணாதே" },
        { text: "சோம்பேறித்தனத்தால் வறுமை வந்த சேருமே", times: 2 },
      ],
    },
    {
      lines: [
        { text: "வேலை செய்ய மனமற்றவன் சாப்பிடவும் கூடாது" },
        { text: "வெட்டிப்பேச்சு பேசியே காலங்கழிக்கக் கூடாது", times: 2 },
        { text: "வேலை ஒன்றைக் கண்டவன் ஆளுகை செய்பவன்" },
        { text: "வேலையற்ற மனிதனோ மாயையிலும் லேசானவன்", times: 2 },
      ],
    },
    {
      lines: [
        { text: "பிரயாசப்படுவதினால் பிரயோஜனம் உண்டாகும்" },
        { text: "பிரயத்தனம் இல்லையேல் பிழைப்பு கெட்டுப் போய்விடும்", times: 2 },
        { text: "வீணர்களின் விருப்பங்கள் அவர்களைக் கொன்றிடும்" },
        { text: "செயல்வீரர் இல்லங்களில் செல்வங்கள் குவிந்திடும்", times: 2 },
      ],
    },
    {
      lines: [
        { text: "வேலையில் முனைபவனை வெற்றிகள் சூழ்ந்திடும்!" },
        { text: "சோம்ப- ன் முழ்குவோனை பாடுகள் வாட்டிடும்", times: 2 },
        { text: "கவலையைப் போக்குவது மதுவல்ல வேலையே" },
        { text: "வேலை செய்து ஓய்ந்திருந்தால் நன்மையிலும் நன்மையே", times: 2 },
      ],
    },
  ],
  chorus: null,
  chorusFrequency: "none",
  startWithChorus: false,
}

export const kalvariMaMalai: Song = {
  title: "கல்வாரி மா மாலையோரம்",
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
  chorus: {
    lines: [
      { text: "கல்வாரி மா மாலையோரம்" },
      { text: "கொடுங்கோர காட்சி கண்டேன்" },
      { text: "கண்ணில் நீர் வழிந்திடுதே" },
      { text: "எந்தன் மீட்பர் இயேசு அதோ" },
    ],
  },
  stanzas: [
    {
      lines: [
        { text: "கல்வாரி மா மாலையோரம்" },
        { text: "கொடுங்கோர காட்சி கண்டேன்" },
        { text: "கண்ணில் நீர் வழிந்திடுதே" },
        { text: "எந்தன் மீட்பர் இயேசு அதோ", times: 2 },
      ],
      noNextChorus: true,
    },
    {
      lines: [
        { text: "எருசலேமின் வீதிகளில்" },
        { text: "இரத்த வெள்ளம் கோலமிட", times: 2 },
        { text: "திருக்கோலம் நிந்தனையால்" },
        { text: "உருக்குலைந்து சென்றனரே", times: 2 },
      ],
    },
    {
      lines: [
        { text: "சிலுவை தன் தோளதிலே" },
        { text: "சிதறும் தன் வேர்வையிலே", times: 2 },
        { text: "சிறுமை அடைந்தவராய்" },
        { text: "நிந்தனை பல சகித்தார்", times: 2 },
      ],
      noNextChorus: true,
    },
    {
      lines: [
        { text: "கல்வாரி மா மாலையோரம்" },
        { text: "கொடுங்கோர காட்சி கண்டேன்" },
        { text: "கண்ணில் நீர் வழிந்திடுதே" },
        { text: "எந்தன் மீட்பர் இயேசு அதோ", times: 2 },
        { text: "|" },
        { text: "எந்தன் மீட்பர் இயேசு அதோ", times: 2 },
      ],
      noNextChorus: true,
    },
  ],
}

export const karthaneEnThunai: Song = {
  title: "கர்த்தனே எம் துணையானீர்",
  startWithChorus: true,
  chorusFrequency: "afterEachStanza",
  intro: {
    lines: [
      {
        text: "[ INTRO ]",
      },
      {
        text: "Rumpumpaa, rumpumpaa, rumpumpaa",
        singer: "boys",
        times: 2,
      },
      {
        text: "Aaa aaa aaa, Aaa aaa aaa, Aaa aaa aaa",
        singer: "girls",
        times: 2,
      },
    ],
  },
  chorus: {
    lines: [
      { text: "கர்த்தனே எம் துணையானீர்" },
      { text: "நித்தமும் எம் நிழலானீர்" },
      { text: "கர்த்தனே எம் துணையானீர்" },
    ],
  },
  stanzas: [
    {
      lines: [
        { text: "எத்தனை இடர் வந்து சேர்ந்தாலும்" },
        { text: "கர்த்தனே அடைக்கல மாயினார்", times: 2 },
        { text: "மனுமக்களில் இவர் போலுண்டோ" },
        { text: "விண் உலகிலும் இவர் சிறந்தவர்" },
      ],
    },
    {
      lines: [
        { text: "பாவி என்றென்னைப் பலர் தள்ளினார்" },
        { text: "ஆவி இல்லை என்றிகழ்ந்தும் விட்டார்", times: 2 },
        { text: "ராஜா உம் அன்பு எனைக் கண்டது" },
        { text: "ஐயா உம்மைப்போல, எங்கும் கண்டதில்லை" },
      ],
    },
    {
      lines: [
        { text: "ஆயிரம் நாவுகள் நீர் தந்தாலும்" },
        { text: "ராஜனே, உமைப் பாடக்கூடுமோ", times: 2 },
        { text: "ஜீவனே உமக்களிக்கின்றேனே" },
        { text: "ஐயா உம்மைப்போல, எங்கும் கண்டதில்லை" },
      ],
    },
    {
      type: "outro",
      noNextChorus: true,
      lines: [
        {
          text: "[ OUTRO ]",
        },
        {
          text: "Rumpumpaa, rumpumpaa, rumpumpaa",
          singer: "boys",
          times: 2,
        },
        {
          text: "Aaa aaa aaa, Aaa aaa aaa, Aaa aaa aaa",
          singer: "girls",
          times: 2,
        },
      ],
    },
  ],
}

const ADHIMANGALA_VOCALS: Intro | Ending = {
  lines: [
    { text: "ஆஆ ஆஆ ஆஆ ஆஆஆ-ஆஆ", singer: "girls" },
    { text: "ச ச ச ச சா - கரி கரி", singer: "boys" },
    { text: "லா லா லா லா லா லா லா", singer: "girls" },
    { text: "ச ச ச - ச நி த ப ம க", singer: "boys" },
    { text: "லா லா லா லா லா லா லா", singer: "girls" },
    { text: "ச ச ச ச", singer: "boys" },
  ],
}
export const adhiMangalaKararane: Song = {
  title: "அதி மங்கல காரணனே",
  chorusFrequency: "none",
  startWithChorus: true,
  intro: ADHIMANGALA_VOCALS as Intro,
  ending: ADHIMANGALA_VOCALS as Ending,
  chorus: {
    lines: [
      { text: "அதி மங்கல காரணனே" },
      { text: "துதி தங்கிய பூரணனே", times: 2 },
      { text: "நரர் வாழ விண் துறந்தோர் ஏழையாய்ப் " },
      { text: "பிறந்த வண்மையே தாரணனே!", times: 2 },
      { text: "அதி மங்கல, அதி மங்கல" },
      { text: "அதி மங்கல காரணனே!", times: 2 },
    ],
  },
  stanzas: [
    {
      type: "interlude",
      lines: [
        { text: "ச நீ ச நீ ச க", singer: "girls" },
        {
          text: "ச நீ த ப க நீ ச",
          singer: "girls2",
          alternateLines: { boys: "சசச நீநீநீ ககக பபப" },
        },
        { text: "ப ம த ரி ம க", singer: "girls" },
        {
          text: "சநீச சநீச நீச",
          singer: "girls2",
          alternateLines: { boys: "ப ம க" },
        },
        { text: "ப த நீ த ச ரி நீ", singer: "girls" },
        { text: "ச ரி க ப ப த நீ ரி ச", times: 2, singer: "boys" },
      ],
    },
    {
      lines: [
        { text: "மதி மங்கின எங்களுக்கும்" },
        { text: "திதி சிங்கினர் தங்களுக்கும்", times: 2 },
        { text: "உனின் மாட்சிமையும் திவ்விய காட்சியும்" },
        { text: "தோன்றிட வையாய் துங்கவனே", times: 2 },
        { text: "அதி மங்கல, அதி மங்கல" },
        { text: "அதி மங்கல காரணனே!", times: 2 },
      ],
    },
    {
      interlude: true,
      lines: [
        { text: "முடி மன்னர்கள் மேடையையும்" },
        { text: "மிகு உன்னத வீடதையும்", times: 2 },
        { text: "நீங்கி மாட்டிடையே பிறந்தாட்டிடையார் தொழ" },
        { text: " வந்தனையோ தரையில்", times: 2 },
        { text: "அதி மங்கல, அதி மங்கல" },
        { text: "அதி மங்கல காரணனே!", times: 2 },
      ],
    },
  ],
}

const tatTara: Line = {
  text: "tat tara tat tara",
  singer: "boys",
  textClassName: "small-text",
  type: "vocals",
}
const maravenInterlude: Stanza = {
  type: "interlude",
  lines: [
    {
      text: "La la la la la",
      times: 4,
    },
  ],
}
const maravenChorusLines: Line[] = [
  { text: "மறவேன் மறவேன் நீர் செய்த நன்மைகள்" },
  { text: "துதிப்பேன் துதிப்பேன் என் முழு இதயத்தோடு", times: 2 },
  { text: "என் கர்த்தர் நல்லவர்.. மிக மிக நல்லவர்" },
  { text: "என்னை விசாரிக்கும் நல் தகப்பனவர்", times: 2 },
]
export const maraven: Song = {
  title: "மறவேன்",
  chorus: {
    lines: maravenChorusLines,
  },
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
  stanzas: [
    {
      lines: [
        { text: "தாயின் வயிற்றில் தோன்றின நாள்முதல்", singer: "girls" },
        { ...tatTara, singer: "boys" },
        { text: "என்னை ஏந்தி சுமந்து காத்த தேவனே", singer: "girls" },
        { ...tatTara, singer: "boys" },
        { text: "(உம்) உள்ளங்கைகளில் என்னை வரைந்து", singer: "girls" },
        { ...tatTara, singer: "boys" },
        {
          text: "உன்தன் கண்மணி போலென்னை காக்கின்றீர்",
          times: 2,
          singer: "girls",
        },
        {
          text: "Aaa Aaa Aaa",
          singer: "boys",
          textClassName: "small-text",
          type: "vocals",
        },
      ],
    },
    maravenInterlude,
    {
      lines: [
        { text: "வெள்ளம் போல் சத்ரு எதிர்த்து வந்தாலும்", singer: "girls" },
        {
          ...tatTara,
          singer: "boys",
          alternateLines: { boys: "வந்தாலும்" },
        },
        { text: "(தேவ)(ஆவி)யானவர் எனக்காய் கொடியேற்றுவீர்", singer: "girls" },
        {
          ...tatTara,
          singer: "boys",
          alternateLines: { boys: "கொடியேற்றுவீர்" },
        },
        { text: "இதுவரை உதவி செய்த நேசரே", singer: "girls" },
        {
          ...tatTara,
          singer: "boys",
          alternateLines: { boys: "நேசரே" },
        },
        { text: "இனியும் உதவி செய்ய வல்லவரே", times: 2, singer: "girls" },
        {
          singer: "boys",
          text: "Aaa Aaa Aaa",
          textClassName: "small-text",
          type: "vocals",
        },
      ],
    },
    maravenInterlude,
    {
      noNextChorus: true,
      lines: [
        { text: "பகைஞர் எதிரே எனக்கு ஓர் பந்தி", singer: "girls" },
        {
          ...tatTara,
          singer: "boys",
          alternateLines: { boys: "ஓர் பந்தி" },
        },
        { text: "ஆயத்தம் செய்த சர்வ வல்லவரே", singer: "girls" },
        {
          ...tatTara,
          singer: "boys",
          alternateLines: { boys: "வல்லவரே" },
        },
        { text: "எண்ணையால் என்னை அபிஷேகம் செய்து", singer: "girls" },
        {
          ...tatTara,
          singer: "boys",
          alternateLines: { boys: "செய்து" },
        },
        {
          text: "என் பாத்திரம் நிரம்பி வழிய செய்கின்றீர்",
          times: 2,
          singer: "girls",
        },
        {
          singer: "boys",
          text: "Aaa Aaa Aaa",
          textClassName: "small-text",
          type: "vocals",
        },
      ],
    },
    {
      noNextChorus: true,
      lines: [
        ...maravenChorusLines,
        { text: "என்னை விசாரிக்கும் நல் தகப்பனவர்", times: 3 },
      ],
    },
  ],
}

export const dhayavu: Song = {
  title: "தயவு",
  intro: {
    lines: [
      { text: "[ INTRO ]" },
      {
        text: "Oooooooo.....",
        times: 2,
      },
      {
        text: "Aa Aaaa.....",
        times: 4,
      },
    ],
  },
  chorus: {
    lines: [
      { text: "உங்க தயவு பெரியதே" },
      { text: "உங்க தயவு சிறந்ததே" },
      { text: "உங்க தயவு என்னை சேதமின்றி பாதுகாத்ததே", times: 2 },
      { text: "ஒரு சேதமின்றி தலைமுறையாய் பாதுகாத்ததே" },
      { text: "தலைமுறைகள் தாண்டி நிற்கும் தயவு" },
      { text: "தலை நிமிர்ந்து வாழ செய்யும் தயவு" },
    ],
  },
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
  stanzas: [
    {
      lines: [
        { text: "தலைமுறைகள் தாண்டி நிற்கும் தயவு" },
        { text: "(என்) தலை நிமிர்ந்து வாழ செய்யும் தயவு", times: 2 },
        { text: "பாரபட்சம் பார்க்காத தயவு" },
        { text: "எளியவனை உயர்த்தி வைக்கும் தயவு" },
        { text: "தலைமுறைகள் தாண்டி நிற்கும் தயவு" },
      ],
    },
    {
      interlude: true,
      lines: [
        { text: "(எனை) குறிபார்த்து எறியப்பட்ட சவுலின் அம்புகள்" },
        { text: "திசை மாறி போக செய்த தயவு பெரியதே", times: 2 },
        { text: "ஒரு அடியின் தூரத்திலே கண்ட மரணத்தை" },
        { text: "தடுத்து நிறுத்தி பாதுகாத்த தயவு பெரியதே" },
        { text: "இந்த தயவை பாட ஜீவன் உள்ளதே" },
      ],
    },
    {
      interlude: true,
      lines: [
        { text: "சுற்றி நின்ற ஜலங்கள் எல்லாம் அமிழ்ந்து போனதே" },
        { text: "என் பேழை மட்டும் பத்திரமாய் மலையில் நின்றதே", times: 2 },
        { text: "மூழ்கும் என்று எதிர்பார்த்த கண்கள் தோற்றதே" },
        { text: "ஏறெடுத்து பார்க்கும் வண்ணம் உயர்த்தி வைத்ததே" },
        { text: "என்னை உயர உயர கொண்டு செல்லுதே" },
      ],
    },
    {
      noNextChorus: true,
      lines: [
        { text: "உங்க தயவு பெரியதே" },
        { text: "உங்க தயவு சிறந்ததே" },
        { text: "உங்க தயவு என்னை சேதமின்றி பாதுகாத்ததே", times: 2 },
        { text: "ஒரு சேதமின்றி தலைமுறையாய் பாதுகாத்ததே" },
      ],
    },
    {
      noNextChorus: true,
      lines: [
        { text: "தலைமுறைகள் தாண்டி நிற்கும் தயவு" },
        { text: "(என்) தலை நிமிர்ந்து வாழ செய்யும் தயவு", times: 2 },
        { text: "பாரபட்சம் பார்க்காத தயவு" },
        { text: "எளியவனை உயர்த்தி வைக்கும் தயவு" },
        { text: "தலைமுறைகள் தாண்டி நிற்கும் தயவு" },
      ],
    },
    {
      noNextChorus: true,
      lines: [
        { text: "[ OUTRO ]" },
        {
          text: "Oooooooo.....",
          times: 2,
        },
        {
          text: "Aa Aaaaa.....",
          times: 4,
        },
      ],
    },
  ],
}

const avarDhayavu = ({ singer }: { singer?: "boys" | "girls" }) => {
  return [
    {
      lines: [
        { text: "அவர் தயவு நம்மேலே", singer },
        { text: "நம் குடும்பங்கள் மேலே", singer },
        { text: "நம் பிள்ளைகள் மேலே", singer },
        { text: "நம் சந்ததிகள் மேலே", singer },
      ],
    },
    {
      lines: [
        { text: "அவர் சமூகம் நம் முன்னே", singer },
        { text: "நம் அருகே நம் பின்னே", singer },
        { text: "அவர் பிரசன்னம் நம்மை மூடுதே", singer },
        { text: "அவர் என்றும் நம்மோடே", singer },
      ],
    },
  ]
}
const amen = {
  lines: [{ text: "ஆமென், ஆமென், ஆமென்", times: 2 }],
}
export const aseervadham: Song = {
  title: "ஆசீர்வாதம் [The Blessing]",
  stanzas: [
    {
      lines: [{ text: "ஆமென், ஆமென், ஆமென்", times: 2 }],
    },
    {
      lines: [
        { text: "கர்த்தர் நம்மை ஆசீர்வதிப்பார்" },
        { text: "அவர் முகத்தை நம்மேல் பிரகாசிப்பார்" },
        { text: "நம்மை காப்பார்" },
        { text: "கிருபையால் மறைப்பார்" },
        { text: "சமாதானம் தருவார்", times: 2 },
      ],
    },
    amen,
    ...avarDhayavu({}),
    ...avarDhayavu({}),
    amen,
    ...avarDhayavu({ singer: "boys" }),
    ...avarDhayavu({ singer: "boys" }),
    amen,
  ],
  chorus: null,
}

export const sarvaSirushtikum: Song = {
  title: "சர்வ சிருஷ்டிக்கும் எஜமானன் நீரே",
  stanzas: [
    {
      lines: [
        { text: "சர்வ சிருஷ்டிக்கும் எஜமானன் நீரே" },
        { text: "சர்வ சிருஷ்டியை காப்பவர் நீரே" },
        { text: "எங்கள் இதயத்தில் உம்மைப் போற்றிடுவோம்" },
        { text: "என்றென்றும் பணீந்து தொழுவோம்", times: 2 },
      ],
    },
    {
      interlude: true,
      lines: [
        { text: "வானம் பூமி ஒழிந்து போனாலும்" },
        { text: "உம் வார்த்தை என்றும் மாறா" },
        { text: "இல்வாழிக்கை அழிந்து மறைந்துபோம்" },
        { text: "விசுவாசி என்றென்றும் நிலைய்ப்பான்", times: 2 },
      ],
    },
    {
      interlude: true,
      lines: [
        { text: "கர்த்தர் கரத்தின் கிரியைகள் நாங்கள்" },
        { text: "கிருபை எங்கள் மேல் ஊற்றுவீரே" },
        { text: "ஆவி ஆத்துமா சரீரம் உம் சொந்தமே" },
        { text: "சாத்தான் தொடராமல் காப்பீர்", times: 2 },
      ],
    },
  ],
  chorus: {
    lines: [
      { text: "ஆ-ஆ-ஆ அல் லே லூ யா", times: 7 },
      { text: "ஆ-ஆ-ஆ ஆ– மேன்" },
    ],
  },
  chorusFrequency: "afterEachStanza",
  startWithChorus: false,
}
